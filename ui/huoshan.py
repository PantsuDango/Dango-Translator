...
# 私人火山设置界面
class HuoshanSetting(QWidget) :

    def __init__(self, object) :

        super(HuoshanSetting, self).__init__()
        self.object = object
    ...
    def ui(self) :
        ...
        # 窗口标题
        self.setWindowTitle("私人火山翻译设置 - 退出会自动保存")
        # 窗口图标
        ...
        self.secret_id_text.setText(self.object.config["huoshanAPI"]["Key"])
        ...
        self.secret_key_text.setText(self.object.config["huoshanAPI"]["Secret"])
        ...
        button.setText("测试")
        button.clicked.connect(lambda: utils.test.testHuoshan(
    ...
    def openTutorial(self) :
        ...
        try :
            url = self.object.yaml["dict_info"]["huoshan_tutorial"]
            webbrowser.open(url, new=0, autoraise=True)
        except Exception :
            self.logger.error(format_exc())
            utils.message.MessageBox("私人火山注册",
                                     "请尝试手动打开此地址:\n%s     " % url)
    ...
    def openQueryQuota(self):
        ...
        url = "https://console.volcengine.com/translate/usage"
        try:
            webbrowser.open(url, new=0, autoraise=True)
        except Exception :
            self.logger.error(format_exc())
            utils.message.MessageBox("私人火山额度查询",
                                     "打开地址失败, 请尝试手动打开此网页下载\n%s     " % url)
    ...
    def closeEvent(self, event) :
        ...
        self.object.config["huoshanAPI"]["Key"] = self.filterNullWord(self.secret_id_text)
        self.object.config["huoshanAPI"]["Secret"] = self.filterNullWord(self.secret_key_text)