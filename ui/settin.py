...
class Settin(QMainWindow) :
    ...
    def setTabTwo(self) :
        ...
        # 私人小牛翻译标签
        label = QLabel(private_translater_tab)
        self.customSetGeometry(label, 320, 210, 35, 20)
        label.setText("小牛:")
        # 私人小牛翻译开关
        self.xiaoniu_private_switch = ui.switch.SwitchOCR(private_translater_tab, sign=self.xiaoniu_private_use, startX=(65-20) * self.rate)
        self.customSetGeometry(self.xiaoniu_private_switch, 370, 210, 65, 20)
        self.xiaoniu_private_switch.checkedChanged.connect(self.changeXiaoniuPrivateTranslaterSwitch)
        self.xiaoniu_private_switch.setCursor(ui.static.icon.SELECT_CURSOR)
        # 私人小牛翻译颜色选择
        self.xiaoniu_private_color_button = QPushButton(qtawesome.icon("fa5s.paint-brush", color=self.xiaoniu_private_color), "", private_translater_tab)
        self.customSetIconSize(self.xiaoniu_private_color_button, 20, 20)
        self.customSetGeometry(self.xiaoniu_private_color_button, 450, 210, 20, 20)
        self.xiaoniu_private_color_button.setStyleSheet("background: transparent;")
        self.xiaoniu_private_color_button.clicked.connect(lambda: self.changeTranslateColor("xiaoniu_private", self.xiaoniu_private_color))
        self.xiaoniu_private_color_button.setCursor(ui.static.icon.SELECT_CURSOR)
        # 私人小牛翻译设置按钮
        button = QPushButton(private_translater_tab)
        ...
        # 翻译备注
        label = QLabel(self.tab_2)
        self.customSetGeometry(label, 20, 370, 250, 20)
        label.setText("点击每种翻译源对应的")
        # 颜色说明
        button = QPushButton(qtawesome.icon("fa5s.paint-brush", color=self.color_2), "", self.tab_2)
        self.customSetIconSize(button, 20, 20)
        self.customSetGeometry(button, 170, 370, 20, 20)
        button.setStyleSheet("background: transparent;")
        # 公共翻译备注
        label = QLabel(self.tab_2)
        self.customSetGeometry(label, 200, 370, 500, 20)
    ...
    def getInitConfig(self):
        ...
        # 私人团子翻译开关
        self.dango_use = self.object.config["dangoUse"]
        if self.dango_use :
        ...
        # 字体颜色 私人团子
        self.dango_color = self.object.config["fontColor"]["dango"]
        # 字体颜色 私人腾讯
    ...
    # 改变私人团子翻译开关状态
    def changeDangoSwitch(self, checked):
        ...
        if checked :
            ...
            self.dango_use = False
            self.translate_list.remove("dango_private")
        self.setTransLabelMessage()
    ...
    # 改变私人阿里云翻译开关状态
    def changeAliyunSwitch(self, checked):

        if checked :
            self.aliyun_use = True
            self.translate_list.append("aliyun_private")
            self.checkTranslaterUse()
        else:
    ...
    # 改变私人小牛翻译开关状态
    def changeXiaoniuPrivateTranslaterSwitch(self, checked):

        if checked :
            self.xiaoniu_private_use = True
            self.translate_list.append("xiaoniu_private")
            self.checkTranslaterUse()
        else:
    ...
    def changeTranslateColor(self, translate_type, color_str) :
        ...
        if translate_type == "youdao" :
            ...
            self.dango_private_color_button.setIcon(qtawesome.icon("fa5s.paint-brush", color=color.name()))
            self.dango_color = color.name()
    ...
    def checkTranslaterUse(self) :
        ...
        for val in self.translate_list :
            ...
            elif val == "dango_private" :
                self.dango_private_switch.mousePressEvent(1)
                self.dango_private_switch.updateValue()
    ...
    def saveConfig(self) :
        ...
        # 字体颜色 私人团子
        self.object.config["fontColor"]["dango"] = self.dango_color
        # 字体颜色 私人腾讯
