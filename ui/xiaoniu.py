...
# 私人小牛设置界面
class XiaoniuSetting(QWidget) :

    def __init__(self, object) :

        super(XiaoniuSetting, self).__init__()
        self.object = object
    ...
    def ui(self) :
        ...
        # 窗口标题
        self.setWindowTitle("私人小牛翻译设置 - 退出会自动保存")
        # 窗口图标
        ...
        self.secret_key_text.setText(self.object.config["xiaoniuAPI"])
        self.secret_key_text.setCursor(ui.static.icon.EDIT_CURSOR)

        # 测试按钮
        button = QPushButton(self)
        self.customSetGeometry(button, 65, 90, 60, 20)
        button.setText("测试")
        button.clicked.connect(lambda: utils.test.testXiaoniu(
            self.object, self.filterNullWord(self.secret_key_text)))
        button.setCursor(ui.static.icon.SELECT_CURSOR)

        # 注册按钮
        button = QPushButton(self)
        self.customSetGeometry(button, 155, 90, 60, 20)
        button.setText("注册")
        ...
        # 查额度按钮
        button = QPushButton(self)
        self.customSetGeometry(button, 245, 90, 60, 20)
        button.setText("查额度")
    ...
    def getInitConfig(self) :
        ...
        self.window_height = int(140 * self.rate)
        # 颜色
        self.color = "#5B8FF9"
    ...
    def openTutorial(self) :
        ...
        try :
            url = self.object.yaml["dict_info"]["xiaoniu_tutorial"]
            webbrowser.open(url, new=0, autoraise=True)
        except Exception :
            self.logger.error(format_exc())
            utils.message.MessageBox("私人小牛注册",
                                     "请尝试手动打开此地址:\n%s     " % url)
    ...
    def openQueryQuota(self):
        ...
        url = "https://niutrans.com/cloud/service/flows"
        try:
            webbrowser.open(url, new=0, autoraise=True)
        except Exception :
            self.logger.error(format_exc())
            utils.message.MessageBox("私人小牛额度查询",
                                     "打开地址失败, 请尝试手动打开此网页下载\n%s     " % url)
    ...
    def closeEvent(self, event) :
        ...
        self.object.config["xiaoniuAPI"] = self.filterNullWord(self.secret_key_text)